...
def checkFiles(path, useGetKmers):
...
def checkExisting(path, folder, useGetKmers):
    existOrNot = path / folder
    if existOrNot.exists() and existOrNot.is_dir(): #ckeck if the dir exist
        if checkFiles(existOrNot, useGetKmers): #if also the needed files already exist
...
def runKmers(inputseq, k, useGetKmers):
    ...
    if checkExisting(KmersDir, inputseq.stem, useGetKmers):
        return # files already exist and are valid. No need to run k-mers again
    else: # files does not exist or they are corrupted -> k-mers will run again for the given genome. 
    ...
    def getKmers(useGetKmers): #the boolean parameter 'useGetKmers' is determining if the k-mers algorith is going to use the getKmers() function or not
        if useGetKmers:
            kmercounter=0
